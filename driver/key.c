...
/******************************************************************************
 * FunctionName : key_longtime_cb
 * Description  : long press 5s timer callback
 * Parameters   : single_key_param *single_key - single key parameter
 * Returns      : none
*******************************************************************************/
LOCAL void ICACHE_FLASH_ATTR
key_longtime_cb(struct single_key_param *single_key)
{
    os_timer_disarm(&single_key->key_longtime);
    ...
    for (i = 0; i < keys->key_num; i++) {
        if (gpio_status & BIT(keys->single_key[i]->gpio_id)) {
            ...
            if (keys->single_key[i]->key_level == 1) {
                ...
                os_timer_disarm(&keys->single_key[i]->key_longtime);
                os_timer_setfn(&keys->single_key[i]->key_longtime, (os_timer_func_t *)key_longtime_cb, keys->single_key[i]);
                os_timer_arm(&keys->single_key[i]->key_longtime, LONG_PRESS_TIME, 0);
